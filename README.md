...
This screenshot shows the **AWS API Gateway** service opened from the AWS Management Console.
API Gateway is used to expose Lambda functions as HTTP endpoints.
In the next steps, this service is used to connect the **PUT**, **GET**, and **DELETE** Lambda functions to REST API routes.

![Open API Gateway](screenshots/15-api-gateway-dashboard.png)
...
- Created routes for **PUT**, **GET**, and **DELETE** methods.
- Mapped each HTTP method to the corresponding Lambda function:
  - **PUT /function** → `functionorder_put`
  - **GET /function** → `functionorder_get`
  - **GET /function/{id}` → `functionorder_get`
  - **DELETE /function/{id}` → `functionorder_delete`
- This configuration allows API Gateway to forward incoming HTTP requests
...
- The HTTP API **functionorder_API** was created successfully.
- Routes were created under the `/function` resource.
- Available routes include:
  - **GET /function**
  - **PUT /function**
  - **GET /function/{id}**
  - **DELETE /function/{id}**
- Each route is connected to the corresponding Lambda function.
- These routes define how incoming HTTP requests are handled by the API.
...
- The default stage **$default** was automatically created.
- Automatic deployment is enabled for the API.
- The **Invoke URL** is generated by API Gateway.
- This Invoke URL is used to access the API endpoints from external clients.
- All PUT, GET, and DELETE requests will be sent using this base Invoke URL.

![API Gateway Stage and Invoke URL](screenshots/18-api-gateway-stage-invoke-url.png)
